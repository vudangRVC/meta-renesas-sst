...
# NOTE: this is kernel-side GPU enablement (pvrsrvkm.ko), added so a
# single-machine rz-cmn core-image-weston can drive the V4H GPU. It builds
# against the team's linux-yocto kernel (virtual/kernel), which carries the
# gsx@fd000000 DT node + drm_file HACK patch added in linux-yocto_6.18.bbappend.
...
SRC_URI:rz-cmn = "${GFX_DRIVER_URL};sha256sum=${GFX_DRIVER_SHA256}"

SRC_URI:append = " file://blacklist.conf"

# Kernel 6.16 removed dma_fence_ops::fence_value_str; the PowerVR fence
# tracepoints still call it and fail to compile once CONFIG_EVENT_TRACING is
# on (as it is in the team's linux-yocto 6.18 alldefconfig). Guard those calls
# by kernel version. See the patch header for details.
SRC_URI:append:rz-cmn = " file://0001-pvr_fence-guard-fence_value_str-removed-in-kernel-6.16.patch"
...
module_do_install() {
    ...
    # UNPACKDIR (not WORKDIR): file:// SRC_URI unpacks to ${UNPACKDIR} on styhead.
